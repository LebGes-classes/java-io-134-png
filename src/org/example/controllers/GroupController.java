...
public class GroupController {
    ...
    //дефолтный без пользовательского ввода
    //учителей подбирает случайно
    public GroupController(int group, ContentController contentController){
        isCreated = true;
        this.group = group;
        this.contentController = contentController;
        schedule = Schedule.makeScheduleForNewGroup(contentController.getSubjects());

        List<Teacher> teachersList = contentController.getTeachers().stream().toList();
        List<Subject> subjects = contentController.getSubjects().values().stream().toList();

        for(int i =0; i < subjects.size(); i++){
            teachers.put(subjects.get(i), teachersList.get(i));
    ...
    public GroupController(int group, ContentController contentController, BufferedReader reader)throws IOException{
        ...
        for(Subject subj: contentController.getSubjects().values()){
            ...
            while(!Pattern.matches("\\d+", answer) || !isTeacherIdCorrect(Integer.valueOf(answer))){
